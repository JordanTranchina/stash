...
- [x] **FIFO Selection + Discussed Dedup**: Select saves oldest-first instead of newest-first so a save can no longer age out of a recency window and be dropped forever, and stamp each save with `podcast_episode_id`/`podcast_discussed_at` once discussed so it's never re-covered by a later episode. (`podcast/extract.py`, `podcast/script.py`, `supabase/migrations/20260726_saves_podcast_discussed.sql`)
